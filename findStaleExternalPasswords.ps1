...
function notifyByEmail ($recipient, $subject, $body, $attachmentPath) {
...
function daysRemainingUntilPasswordMustBeChanged {
	...
	foreach($SingleUser in $HomeOfficeUser) {
        ...
        if ($daysRemaining.Days -lt 7) {
            ...
            #log $recipient,$subject,$body,$attachmentPath,$daysRemaining.Days

            notifyByEmail $recipient $subject $body $attachmentPath
        } 
